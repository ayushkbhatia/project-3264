...
// (and, in the port, stops again when it leaves), and each rebuild reads the box's live
// clientWidth (frame()), so they are responsive by reconstruction rather than by viewBox scaling.
//
// Honours prefers-reduced-motion: with it set, the vignette is built once in its resting
// state and never cycles (the port rebuilds that resting state when the box changes width).
//
...
//
// PORT NOTE: the three-loader import the extraction template added here was unused (this
// system is plain SVG + WAAPI) and has been dropped, as have the template's unused `done` flag
// and the rAF cancel in destroy() (this system never requests a frame).
...
export class Vignettes {
  ...
  startVignettes() {
    // PORT FIX (Strict Mode): useMotionSystem defers start() by a microtask, so a system that
    // Strict Mode has already destroyed can still be started. Bail, or it would leave its own
    // timers and observers running alongside the live instance's.
    ...
    // PORT FIX (resize): the prototype attached its ResizeObserver below the reduced-motion
    // return, so with reduced motion a diagram kept the width it was first drawn at and spilled
    // across the neighbouring cards after a resize or rotation. And the observer's first
    // notification, which fires on observe() with no resize at all, repainted every box still
    // ~40ms into its first animated cycle: a two-frame blink, then 6-7s of static diagrams. Now
    // all three boxes are observed in both modes, and a box is repainted (still, as before) only
    // once the width it was drawn at no longer matches it.
    const stale = (box) => { const s = box.firstChild; return !!s && +s.getAttribute('width') !== Math.max(200, box.clientWidth); };
    if ('ResizeObserver' in window) {
      const repaint = () => { this._still = true; set.forEach(([ref, kind]) => { if (ref.current && stale(ref.current)) build(kind, ref.current); }); this._still = false; };
      // (the debounce handle lives on the instance so destroy() can clear it)
      this._ro = new ResizeObserver(() => {
        if (!set.some(([ref]) => ref.current && stale(ref.current))) return;
        clearTimeout(this._roT);
        this._roT = setTimeout(repaint, 280);
      });
      set.forEach(([ref]) => { if (ref.current) this._ro.observe(ref.current); });
    }
    // PORT FIX (first cycle): the resting still is painted up front only for reduced motion. With
    // motion it was only ever seen for the 240ms before the first cycle, as a finished diagram
    // that cut to blank; now the first thing drawn in the box is the cycle itself.
    if (this.reduced) { paintStill(); return; }
    const started = [false, false, false];
    const start = (i) => {
      const box = set[i][0].current;
      if (!box || started[i]) return;
      started[i] = true;
      // PORT FIX: one live timer per vignette (the prototype pushed a new id every cycle, so the
      // list grew for as long as the page stayed open). Same schedule.
      const cycle = () => { build(set[i][1], box); this.timers[i] = setTimeout(cycle, set[i][2]); };
      this.timers[i] = setTimeout(cycle, 240);
    };
    // PORT FIX (off-screen cost): the prototype started a vignette on first sight and never
    // stopped it, so after one look at the hero all three rebuilt and animated every 6-7s for as
    // long as the page stayed open (~60 style recalcs/s with the hero 10,000px away). Now a box
    // that leaves the (140px-extended) viewport stops: its timer is cleared and its animations
    // cancelled, which leaves the cycle's opening frame and no further work. Coming back starts
    // a fresh cycle, 240ms in, exactly as on first view.
    const stop = (i) => {
      const box = set[i][0].current;
      if (!box || !started[i]) return;
      started[i] = false;
    ...
    if ('IntersectionObserver' in window) {
      this._io = new IntersectionObserver((es) => es.forEach((e) => {
        ...
        if (i < 0) return;
        if (e.isIntersecting) start(i); else stop(i);
      }), { rootMargin: '140px' });
  ...
  destroy() {
    ...
    // PORT FIX: the extraction dropped the prototype's ResizeObserver disconnect, and the
    // pending debounced repaint was unreachable; a destroyed system could still repaint.
