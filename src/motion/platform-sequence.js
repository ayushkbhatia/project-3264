...
export class PlatformSequence {
  ...
  async phSetupGL() {
    ...
    // PORT: destroy() can land while three is loading (Strict Mode, the 768px media gate).
    // Build nothing for a dead instance — no renderer, no context on the shared canvas.
    if (this._dead) return false;

    const scene = new T.Scene();
    const cam = new T.PerspectiveCamera(30, 1, 0.1, 400);
    // PORT FIX: the constructor throws when WebGL is unavailable (GPU-less VDI, a blocklisted GPU,
    // 3D APIs disabled by policy). Report it as "no GL" like a failed three import, as hero-art.js
    // and intro-relief.js do; the panels, rail and caption run without it (see phStep).
    let r;
  ...
  // PORT FIX: phStep is split into a DOM half (p, act weights, the three panels, rail, caption,
  // counter) and a GL half (canvas size, phScene, hover, render, labels). The DOM half used to
  // wait for phGL, so without WebGL, with three failing to load, or while three was still
  // downloading, the section sat on Assess beside an empty stage. Nothing in it needs WebGL:
  // the shipped diagrams are the three DOM panels. It now runs from start(); the GL half joins
  // once phSetupGL has succeeded.
  phStep(ms) {
    const g = this.phGL, track = this.phTrack.current;
    if (!track) return;
    const rect = track.getBoundingClientRect(), vh = window.innerHeight;
    // PORT FIX: stands in for the old `!cw || !ch` bail on the canvas — a track with no box
    // (display:none) is not laid out and has no progress to read.
    ...
    // PORT FIX: the first step that lands is a snap (see phHold).
    const snap = this._phActive === undefined;
    const release = snap ? this.phHold() : null;
    ...
    if (active !== this._phActive) {
      ...
      [this.phP0, this.phP1, this.phP2].forEach((ref, i) => {
        const n = ref.current;
        // PORT FIX: on the snap, phHold keeps the transition at none; arming it here would cross-fade.
        if (n) { if (!snap) n.style.transition = "opacity .42s cubic-bezier(.16,1,.3,1)"; n.style.opacity = i === active ? "1" : "0"; n.style.pointerEvents = i === active ? "auto" : "none"; }
      });
      [this.phR0, this.phR1, this.phR2].forEach((ref, i) => {
        ...
        // PORT: the rail tabs are <button>s in the app; expose the active act to assistive tech.
        n.setAttribute("aria-pressed", i === active ? "true" : "false");
      });
    ...
    // ---- GL half: only once phSetupGL has succeeded ----
    if (!g) return;
    ...
    // PORT FIX: with the diagram hooks empty there is nothing to draw, and rendering anyway
    // cleared and composited a full-stage transparent canvas every frame. Render only while an
    // act holds something, plus one last frame after it empties so nothing stale stays up.
    const drawable = g.acts.some((grp) => grp.children.length > 0);
    if (drawable || g.drew) g.r.render(g.scene, g.cam);
    g.drew = drawable;
  ...
  // PORT FIX: the server HTML always shows act 0, but a reload, back-navigation or a remount
  // across the media gate can land the reader in the middle of the track. The first step that
  // lands therefore snaps the pane to its act instead of cross-fading into it (the old behaviour
  // double-exposed Assess over Build for ~0.4s and re-ticked the Build boxes from empty). Every
  // transition in the pin is held at none while that step writes; the returned release commits
  // the new values with a style flush, then puts the transitions back for scroll-driven changes.
  phHold() {
    const pin = this.phPin.current;
    if (!pin) return null;
    const els = [pin, ...pin.querySelectorAll("*")];
    const was = els.map((n) => n.style.transition);
    els.forEach((n) => { n.style.transition = "none"; });
    return () => {
      void pin.offsetHeight;
      els.forEach((n, i) => { n.style.transition = was[i]; });
    };
  }

  // PORT FIX: the loop used to re-queue every frame for the life of the page (the reference's
  // does too): 60 wake-ups a second at the top and bottom of the page, where phStep only bails.
  // It now runs while the track is within a viewport of the screen, which is wider than
  // phStep's own 200px band, and parks outside it; the observer re-arms it on the way back.
  phWatch() {
    const track = this.phTrack.current;
    if (!track || typeof IntersectionObserver === "undefined") { this._phNear = true; this.phArm(); return; }
    this._phIO = new IntersectionObserver((entries) => {
      this._phNear = entries[entries.length - 1].isIntersecting;
      if (this._phNear) this.phArm();
    }, { rootMargin: "100% 0px" });
    this._phIO.observe(track);
  }

  phArm() {
    if (this.phRaf || this._dead) return;
    const loop = (ms) => {
      this.phRaf = null;
      if (this._dead) return;
      this.phStep(ms);
      if (this._phNear) this.phRaf = requestAnimationFrame(loop);
    };
  ...
  phBuildAssess() {
    ...
    const counter = el("div", "flex:none; font-family:var(--font-plex-mono),'IBM Plex Mono',ui-monospace,monospace; font-size:11.5px; color:var(--mut); font-variant-numeric:tabular-nums", "0 / 4");
    ...
    const lineEls = A.lines.map((l) => {
      const row = el("div", "display:none; gap:12px; padding:2px 0; font-family:var(--font-plex-mono),'IBM Plex Mono',ui-monospace,monospace; font-size:11px; line-height:1.55");
  ...
  phBuildCaps() {
    ...
    headL.appendChild(headline);
    head.appendChild(headL);
    const counter = el("div", "flex:none; font-family:var(--font-plex-mono),'IBM Plex Mono',ui-monospace,monospace; font-size:11.5px; color:var(--mut); font-variant-numeric:tabular-nums", "0 / 7");
    head.appendChild(counter);
    host.appendChild(head);
    ...
    const capEls = CAPS.map((c) => {
      ...
      const tg = el("div", "margin:2px 0 0; font-family:var(--font-plex-mono),'IBM Plex Mono',ui-monospace,monospace; font-size:10px; color:var(--mut)", "wave " + c.wave + " \u00b7 " + c.tag);
    ...
    const waveEls = [1, 2, 3].map((w) => {
      ...
      const lab = el("div", "margin:6px 0 0; font-family:var(--font-plex-mono),'IBM Plex Mono',ui-monospace,monospace; font-size:10.5px; color:var(--mut)", "Wave " + w + " \u00b7 wk " + ((w - 1) * 4 + 2));
  ...
  phBuildRun() {
    ...
    headL.appendChild(headline);
    head.appendChild(headL);
    const state = el("div", "flex:none; font-family:var(--font-plex-mono),'IBM Plex Mono',ui-monospace,monospace; font-size:11.5px; color:" + PASS, "passing");
    ...
    const chip = el("div", "position:absolute; z-index:2; right:0; padding:1px 4px; background:#FFFFFF; font-family:var(--font-plex-mono),'IBM Plex Mono',ui-monospace,monospace; font-size:10px; color:" + FAIL, "threshold 0.94");
    ...
    const axis = el("div", "flex:none; display:flex; justify-content:space-between; margin:7px 0 0; font-family:var(--font-plex-mono),'IBM Plex Mono',ui-monospace,monospace; font-size:10px; color:var(--mut)");
    ...
    const pScore = el("span", "flex:none; font-family:var(--font-plex-mono),'IBM Plex Mono',ui-monospace,monospace; font-size:11px; color:var(--mut)", "");
    ...
    const slices = SL.map((sl) => {
      ...
      const dl = el("div", "margin:2px 0 0; font-family:var(--font-plex-mono),'IBM Plex Mono',ui-monospace,monospace; font-size:10px", "");
  ...
  async initPhases() {
    // PORT: in React this instance can be destroyed before start() runs (Strict Mode builds,
    // destroys and rebuilds every system on the same DOM) — a dead instance touches nothing.
    if (this._dead || !this.phTrack.current) return;
    this.phLayout();
    ...
    this.phBuildCaps();
    this.phBuildRun();
    // PORT: next/font swaps the webfonts in; if they land after this first measure, the panel
    // heights and fit tiers were taken on fallback metrics. Measure again once they are in.
    if (document.fonts && document.fonts.status !== "loaded") {
      document.fonts.ready.then(() => { if (!this._dead) this.phLayout(); });
    }
    // PORT FIX: step the DOM half now, in the same task as the build, and start the loop before
    // three is even requested. A restored scroll position shows its act on the next paint, and
    // the sequence follows scroll whether or not WebGL ever arrives.
    this.phStep(performance.now());
    this.phWatch();
    const ok = await this.phSetupGL();
    // PORT: destroyed while three was loading — the resize listener is already gone; stop here.
    if (this._dead) return;
    // PORT FIX: no GL is no longer a one-off static Assess paint; the loop above carries on without it.
    if (!ok) return;
    this.phLayout();
  ...
  destroy() {
    // PORT: checked by initPhases/phSetupGL after every await and by the rAF loop, so a start()
    // that resolves after destroy() leaves no renderer, loop or listener behind.
    this._dead = true;
    ...
    if (this._phTween) cancelAnimationFrame(this._phTween);
    this.phRaf = this._phTween = null;
    // PORT FIX: the observer that re-arms the loop (phWatch).
    if (this._phIO) this._phIO.disconnect();
    this._phIO = null;
    ...
    if (this.phGL) {
      ...
      this.phGL.r.dispose();
      // PORT: once the canvas has left the document (a real unmount) release its context now
      // rather than at GC. While it is still attached (Strict Mode, the media gate) the next
      // instance renders into this same canvas, and a lost context cannot be taken back.
      if (!this.phGL.cv.isConnected) this.phGL.r.forceContextLoss();
      this.phGL = null;
    }
    if (this._ro) this._ro.disconnect();
    // PORT: drop the DOM this instance built (and the row listeners bound to it) so a rebuilt
    // instance starts from empty containers. phBuild* clear their own hosts too; the label
    // layer is only ever appended to.
    [this.phLabels, this.phAssess, this.phBuildP, this.phRun].forEach((ref) => { if (ref.current) ref.current.textContent = ""; });
    if (this.phTip.current) this.phTip.current.style.opacity = "0";
    this._as = this._bd = this._rn = null;
  }
}
